...
| `tests/inference.test.mjs` | 最新フレームの送信、1件制限、頻度上限、停止・再開、性能指標の失効 |
| `tests/mask.test.mjs` | マスクの位置・向き・サイズ、余白と左右反転の座標変換、不正な座標の除外 |
| `tests/test_app.py` | ローカル HTTP、配信範囲、Host / Origin / セッショントークン、終了処理 |
| `tests/browser/app.test.mjs` | 実ブラウザー・実モデル、カメラ制御、障害案内、演出モード、ローカル画像・動画背景、ポーズ維持・解除、画面サイズ |

ブラウザーテストでは人工カメラを使用し、利用者の実カメラにはアクセスしません。手の認識は、出典を記載した公開テスト画像でも検証します。生成したスクリーンショットは `test-results/` に保存され、Git の対象外です。

OBS用表示は、3種類の縦横比・リサイズ・マスクの座標・ジェスチャー演出の継続・カメラ切断・通常表示への復帰をブラウザーテストで検証します。OBS 本体による取り込みは別途、実機で確認してください。

性能関連の回帰テストでは、遅延した推論結果とImageBitmap生成、一時停止・タブ非表示をまたぐ結果の破棄、表示オフ時のCanvas操作、同じ座標の静止マスクの描画回数、カメラなしでの動画マスク更新も検証します。実機の性能を比較する場合は、同じ入力・解像度・認識頻度上限を使い、モデルの準備とウォームアップ後に診断情報を確認してください。ヘッドレスの人工カメラによる結果を、実カメラやOBS併用時の速度保証として扱わないでください。
